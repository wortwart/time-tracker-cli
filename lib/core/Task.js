...
var Task = function () {
	...
	_createClass(Task, [{
		...
		value: function _calcTime(t) {
			if (t.stop) return (0, _moment2.default)(t.stop).diff((0, _moment2.default)(t.start), 'seconds');else return (0, _moment2.default)().diff((0, _moment2.default)(t.start), 'seconds');
		}
	}, {
		key: '_modifyTiming',
		...
		value: function start(description) {
			...
			return new Promise(function (resolve, reject) {
				...
				if (timing && timing.start && !timing.stop) reject(_this.error('This task already started.'));
				_this.task.timings.push({ start: (0, _moment2.default)().toDate() });
		...
		value: function pause() {
			...
			return new Promise(function (resolve, reject) {
				...
				if (!timing) reject(_this2.error('This task is not started, start it.'));
				if (timing && timing.start && timing.stop) reject(_this2.error('This task are ended/paused, unpaused it.'));
		...
		value: function unpause() {
			...
			return new Promise(function (resolve, reject) {
				...
				if (!timing) reject(_this3.error('This task is not started, start it'));
				if (timing && timing.start && !timing.stop) reject(_this3.error('This task is started, pause/stop first'));
				_this3.task.timings.push({ start: (0, _moment2.default)().toDate() });
		...
		value: function stop(description) {
			...
			return new Promise(function (resolve, reject) {
				...
				if (!timing || timing && !timing.start) reject(_this4.error('This task is not started, start it'));
				if (timing.start && timing.stop) reject(_this4.error('This task already ended, start/unpause it.'));
		...
		value: function log(operation) {
			if (!this.task.log) this.task.log = [];
